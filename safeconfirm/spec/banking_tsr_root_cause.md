...
**Status:** 2026-09-16 · code review + evaluator checks (no local `runs/bridge` banking logs in repo)  
...
3. **Benign case** — Single-step explicit transfer (`savings@bank.com`, $50). Failure here implicates **model/planner**, not poison evaluator. Run: `./util_scripts/run_banking_benign_check.sh`.
