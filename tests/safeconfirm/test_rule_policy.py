...
def test_baseline_allow():
    ...
    selected = select_intervention(
        analysis, load_registry(_registry_path()), "send_email", "baseline_allow"
    )
...
def test_baseline_block():
    analysis = _analysis(has_gap=True)
    selected = select_intervention(
        analysis, load_registry(_registry_path()), "send_email", "baseline_block"
    )
...
def test_baseline_vague():
    analysis = _analysis(has_gap=True)
    selected = select_intervention(
        analysis, load_registry(_registry_path()), "send_email", "baseline_vague"
    )
...
def test_rule_v1_untrusted_without_role_uses_source_aware():
    analysis = _analysis(has_gap=True)
    selected = select_intervention(analysis, load_registry(_registry_path()), "send_email", "rule_v1")
